						...
						<div class="card-body">
							<form:form modelAttribute="turma" action="insereTurma" method="POST">
								<div class="form-group">
									...
									<form:input path="identificacao" class="form-control" id="inputIdentificacao"
										type="text" placeholder="Ex: 913-B" required="true" minlength="3" maxlegth="8" />
								</div>
								<div class="form-group">
									<label for="selectCurso">Curso</label>
									<form:select id="selectCurso" class="selectpicker form-control" data-live-search="true"
										title="Curso" path="curso" required="true">
										<c:forEach items="${cursos}" var="c">
											<form:option value="${c.id}">${c.nome} - ${c.modalidade.descricao}</form:option>
										</c:forEach>
									</form:select>
								</div>
								<div class="form-row">
									<div class="form-group col-md-6">
										...
										<form:select path="turno" class="form-control" id="selectTurno">
											<c:forEach items="${turnos}" var="t">
												<form:option value="${t}">${t.turno}</form:option>
											</c:forEach>
										</form:select>
									</div>
									<div class="form-group col-md-6">
										<label for="selectPeriodo">Período</label>
										<form:select id="selectPeriodo" class="selectpicker form-control"
											data-live-search="true" path="periodo" required="true">
											<c:forEach items="${periodos}" var="p">
												<form:option value="${p.id}">${p.toString()}</form:option>
											</c:forEach>
										</form:select>
									</div>
								</div>
